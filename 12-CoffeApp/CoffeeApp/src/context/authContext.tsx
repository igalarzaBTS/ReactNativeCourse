...
import cafeAPI from "../api/CoffeeAPI";
...
export const AuthProvider = ({ children }: any) => {
  ...
  const signUp = async (info: RegisterData) => {
    ...
    try {
      ...
      dispatch({
        type: 'addError',
        payload: error.response.data.errors[0].msg || 'Please, review your credentials'
      });
    }
  }
  ...
  const signIn = async (info: LoginData) => {
    try {
      ...
      dispatch({
        type: 'addError',
        payload: error.response.data.msg || 'Credentials No valids'
      })
    }
  }
